module SharedViewHelper
  ...
  def category_subcategory_shares(object, owner)
    ...
    obj_shares = object.try(:shares) || object.includes(:shares).map(&:shares).flatten.uniq
    category = object.try(:category) || object.try(:first).try(:category)
    return obj_shares.flatten if category.nil? || owner.nil?
    category_shares = owner.shares.includes(:shareable).reject{ |x| x.shareable_type.nil? }.select { |sh| sh.shareable == category }
